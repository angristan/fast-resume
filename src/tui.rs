...
use crate::cache::{load_sessions, save_sessions};
use crate::config::{AGENT_ORDER, AGENTS, VERSION};
use crate::model::Session;
use crate::scan::refresh_sessions_incremental;
...
impl AppState {
    fn new(
        ...
        let mut state = Self {
            ...
            yolo,
            scanning: true,
            status: "loading cache; refreshing session index".to_string(),
            last_search_ms: 0.0,
            show_preview: true,
            modal: None,
            images,
...
pub fn run_tui(
    ...
    let cached = load_sessions().unwrap_or_default();
    let (scan_tx, scan_rx) = mpsc::channel();
    let refresh_seed = cached.clone();
    thread::spawn(move || {
        let start = Instant::now();
        let refreshed = refresh_sessions_incremental(refresh_seed);
        ...
        let _ = scan_tx.send(ScanMessage::Finished {
            ...
            new_or_modified: refreshed.new_or_modified,
    ...
    let mut state = AppState::new(query, agent_filter, yolo, cached, images);
        ...
        while let Ok(message) = scan_rx.try_recv() {
            match message {
                ScanMessage::Finished {
                    ...
                    state.engine.update(sessions);
                    state.scanning = false;
                    state.status = format!(
                        "refresh complete: {count} sessions, {new_or_modified} changed, {deleted} deleted in {:.1}ms",
...
mod tests {
    ...
    fn plain_j_and_k_type_into_search() {
        let mut state = AppState::new(String::new(), None, false, Vec::new(), None);
